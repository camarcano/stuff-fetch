...
def parse_array_from_fangraphs_html(start_date,end_date):
    ...
    """
    # parse input
    PITCHERS_URL = "https://www.fangraphs.com/leaders.aspx?pos=all&stats=pit&lg=all&qual=0&type=36&season=2021&month=1000&season1=2015&ind=0&team=&rost=&age=&filter=&players=0&startdate={}&enddate={}&page=1_2000".format(start_date, end_date)
    # request the data
...
periods = [('2022-03-01', '2022-05-31'), ('2022-06-01', '2022-07-31'), ('2022-08-01', '2022-10-31')]
...
for i in range(0, len(dataset)):
    dataset[i] = dataset[i][['Name', 'IP', 'Stuff+', 'Location+', 
                       'Pitching+']].copy()

df_merged = reduce(lambda  left,right: pd.merge(left,right,on=['Name'],
                                            how='outer'), dataset).fillna(0)
